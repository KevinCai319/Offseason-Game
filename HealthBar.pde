public class HealthBar extends GameObject {
  ...
  public HealthBar(float XPos, float YPos, float Length, float Width, float borderWidth, float min, float max, color borderColor, color emptyColor, color fullColor) {
    this(new PVector(XPos,YPos),new PVector(Length,Width,borderWidth), new PVector(min,max),borderColor,emptyColor,fullColor);
  }

  public HealthBar(PVector Pos, PVector Dim, PVector Range, color borderColor, color emptyColor, color fullColor) {
    this.Pos = Pos;
    this.Dim = Dim;
    this.Range = Range;
